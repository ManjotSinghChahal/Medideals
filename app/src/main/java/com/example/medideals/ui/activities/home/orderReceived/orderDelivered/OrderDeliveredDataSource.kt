...
class OrderDeliveredDataSource : PageKeyedDataSource<String, OrderReceivedResult>() {
    ...
    override fun loadInitial(params: LoadInitialParams<String>, callback: LoadInitialCallback<String, OrderReceivedResult>) {
        GlobalScope.launch {
            try {
                val response = apiService.fetchOrderDelivered(vendor_id = SharedPrefUtil.getInstance()!!.getUserId(),page_no = OrderReceived.currentPageOrderRec.toString()).await()
                when{
